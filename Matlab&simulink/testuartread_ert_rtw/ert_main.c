...
void rt_OneStep(void)
{
  ...
  __enable_irq();
  currentTime = (extmodeSimulationTime_T) testuartread_M->Timing.taskTime0;
  testuartread_step();
...
int main(int argc, char **argv)
{
  ...
  rtmSetErrorStatus(testuartread_M, 0);
  ...
  testuartread_initialize();
  __disable_irq();
  __enable_irq();

  /* External Mode initialization */
  errorCode = extmodeInit(testuartread_M->extModeInfo, &rtmGetTFinal
    (testuartread_M));
  if (errorCode != EXTMODE_SUCCESS) {
  ...
  if (errorCode == EXTMODE_SUCCESS) {
    ...
    if (extmodeStopRequested()) {
      rtmSetStopRequested(testuartread_M, true);
    }
  }
  ...
  runModel =
    ...
    !rtmGetStopRequested(testuartread_M);
  __enable_irq();
  __enable_irq();
  while (runModel) {
